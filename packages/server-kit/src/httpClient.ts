/**
 * Retrying HTTP client for Node services: timeout + bounded exponential backoff
 * with full jitter and consistent error types.
 *
 * Moved verbatim from the adapter's `src/utils/httpClient.ts` so the retry
 * semantics (retryable status codes, network-error/timeout handling, backoff
 * formula) are byte-for-byte preserved. It is fetch-based and uses
 * `AbortController` / `DOMException`, both available in Node's global scope.
 */
