 ...
 * useDispatchFlow and useGeofenceManager: a mode can now be exited from outside
 * the owning hook (entering another mode, a replay starting, …), so each mode
 * clears its own leftover state on the edge. `onFall` is read through a ref so
 * it can change identity without resubscribing / re-firing.
 */
