...
describe("validateSqlQuery", () => {
  ...
  describe("accepts valid SELECT queries", () => {
    it("simple SELECT", () => {
      expect(
        validateSqlQuery("SELECT id, name, lat, lng FROM vehicles"),
      ).toEqual({ valid: true });
    });
    ...
    it("SELECT with WHERE clause", () => {
      expect(
        validateSqlQuery(
          "SELECT id, name FROM vehicles WHERE status = 'active'",
        ),
      ).toEqual({ valid: true });
    });
    ...
    it("SELECT with subquery", () => {
      expect(
        validateSqlQuery(
          "SELECT * FROM (SELECT id, name FROM vehicles) AS sub",
        ),
      ).toEqual({ valid: true });
    });
    ...
    it("SELECT with ORDER BY, LIMIT, OFFSET", () => {
      expect(
        validateSqlQuery(
          "SELECT id, name FROM vehicles ORDER BY name LIMIT 100 OFFSET 0",
        ),
      ).toEqual({ valid: true });
    });
    ...
    it("SELECT with leading whitespace", () => {
      expect(
        validateSqlQuery("   SELECT id FROM vehicles"),
      ).toEqual({ valid: true });
    });

    it("lowercase select", () => {
      expect(
        validateSqlQuery("select id, name from vehicles"),
      ).toEqual({ valid: true });
    });
    ...
    it("SELECT with DISTINCT", () => {
      expect(
        validateSqlQuery("SELECT DISTINCT name FROM vehicles"),
      ).toEqual({ valid: true });
    });
    ...
    it("column names containing keyword substrings (updated_at, created_by)", () => {
      expect(
        validateSqlQuery(
          "SELECT id, updated_at, created_by, deleted_flag FROM vehicles",
        ),
      ).toEqual({ valid: true });
    });
  });
  ...
  describe("rejects non-SELECT queries", () => {
    ...
    it("INSERT statement", () => {
      const result = validateSqlQuery(
        "INSERT INTO vehicles (id, name) VALUES (1, 'test')",
      );
      expect(result.valid).toBe(false);
    ...
    it("UPDATE statement", () => {
      const result = validateSqlQuery(
        "UPDATE vehicles SET name = 'hacked' WHERE 1=1",
      );
      expect(result.valid).toBe(false);
      expect(result.reason).toMatch(/SELECT/);
    });
  ...
  describe("rejects SELECT queries with embedded dangerous keywords", () => {
    it("SELECT … ; DROP TABLE", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles; DROP TABLE vehicles",
      );
      expect(result.valid).toBe(false);
    });
    ...
    it("SELECT INTO OUTFILE", () => {
      const result = validateSqlQuery(
        "SELECT * FROM vehicles INTO OUTFILE '/tmp/dump.csv'",
      );
      expect(result.valid).toBe(false);
    ...
    it("SELECT … UNION INSERT", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles UNION INSERT INTO logs VALUES (1)",
      );
      expect(result.valid).toBe(false);
    });
    ...
    it("SELECT with EXEC", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles WHERE EXEC xp_cmdshell('dir')",
      );
      expect(result.valid).toBe(false);
    ...
    it("SELECT with CREATE", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles; CREATE TABLE evil (id INT)",
      );
      expect(result.valid).toBe(false);
    });
    ...
    it("SELECT with ALTER", () => {
      const result = validateSqlQuery(
        "SELECT 1; ALTER TABLE vehicles ADD COLUMN pwned TEXT",
      );
      expect(result.valid).toBe(false);
    });
    ...
    it("SELECT with TRUNCATE", () => {
      const result = validateSqlQuery(
        "SELECT 1; TRUNCATE TABLE vehicles",
      );
      expect(result.valid).toBe(false);
    });
    ...
    it("SELECT with MERGE", () => {
      const result = validateSqlQuery(
        "SELECT 1 WHERE MERGE INTO t USING s ON t.id = s.id",
      );
      expect(result.valid).toBe(false);
    ...
    it("SELECT with DELETE (case-insensitive)", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles; delete FROM vehicles",
      );
      expect(result.valid).toBe(false);
    });
  ...
  describe("rejects queries containing SQL comments", () => {
    it("line comment (--)", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles -- this is a comment",
      );
      expect(result.valid).toBe(false);
    ...
    it("block comment start (/*)", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles /* hidden */",
      );
      expect(result.valid).toBe(false);
      expect(result.reason).toMatch(/comment/i);
    });
    ...
    it("block comment used to obfuscate keyword", () => {
      const result = validateSqlQuery(
        "SELECT id FROM vehicles; DR/**/OP TABLE vehicles",
      );
      expect(result.valid).toBe(false);
      expect(result.reason).toMatch(/comment/i);
    });
